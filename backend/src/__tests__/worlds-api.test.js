...
describe('worlds API', () => {
  it('can create world', async () => {
    ...
    const world = await api.createWorld({
      info: {name: 'alpha', resolution: 3},
      data: {heights: [1, 2, 3]}
    });

    expect(world.id).toBeDefined()
    expect(world.info).toEqual(
      expect.objectContaining({name: 'alpha', resolution: 3})
    )
  });
  ...
  it('can find worlds', async () => {
    ...
    const world2 = await api.createWorld({
      info: {name: 'beta', resolution: 2},
      data: {heights: [4, 5, 6]}
    });

    const result = await api.findWorlds();
    const foundWorld = _.find(result, {id: world.id});
    expect(_.get(foundWorld, 'info.name')).toBe(world.info.name);
    const foundWorld2 = _.find(result, {id: world2.id});
    expect(_.get(foundWorld2, 'info.name')).toBe(world2.info.name);
  });
