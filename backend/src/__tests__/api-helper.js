...
export async function apiHelper() {
  ...
  return {
    ...
    createWorld: params => client.post('/worlds/', params).then(assertStatus(201)),
    findWorlds: params =>
      client.get('/worlds', {params}).then(assertStatus(200)),
    deleteTestData: params => client.delete('/worlds/0/testdata').then(assertStatus(200))
  }
}
...
const startServer = memoize(async () => (await createServer()).listen())
